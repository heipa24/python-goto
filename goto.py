...
# Provide dummy placeholders so IDEs and type checkers won't flag
# "label" and "goto" as undefined. These objects only need to
# support attribute access (e.g. `label.start`) which will be
# compiled into bytecode that `with_goto` later patches out.
class _LabelGotoStub:
    def __getattr__(self, name):
        return self


# Explicitly export these symbols so `from goto import label, goto`
# satisfies linters and IDEs. Annotate as `Any` for type checkers.
label: Any = _LabelGotoStub()
goto: Any = _LabelGotoStub()


__all__ = ['with_goto', 'label', 'goto']


class _Bytecode:
    def __init__(self):
        # Detect bytecode layout for multiple Python versions.
        # Python 3.11 changed the instruction layout so every instruction
        # is followed by a single-byte oparg and instruction offsets are
        # measured in 2-byte units. Handle that explicitly.
